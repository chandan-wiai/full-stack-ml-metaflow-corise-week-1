...
cutoff = 4
labeling_function = lambda row: 1 if row['rating'] >= cutoff else 0
...
class BaselineNLPFlow(FlowSpec):
    ...
    def baseline(self):
        ...
        labels = self.traindf.label.tolist()
        self.valdf['preds'] = statistics.mode(labels)
    ...
    def end(self):
        ...
        fp_df = self.valdf[(self.valdf.preds == 1) & (self.valdf.label == 0)]
        # TODO: display the false_positives dataframe using metaflow.cards
        current.card.append(
            Table.from_dataframe(fp_df)
        )
        # Documentation: https://docs.metaflow.org/api/cards#table
        
        current.card.append(Markdown("## Examples of False Negatives"))
        # TODO: compute the false negatives predictions where the baseline is 0 and the valdf label is 1. 
        fn_df = self.valdf[(self.valdf.preds == 0) & (self.valdf.label == 1)]
        # TODO: display the false_negatives dataframe using metaflow.cards
        current.card.append(
            Table.from_dataframe(fn_df)
        )
